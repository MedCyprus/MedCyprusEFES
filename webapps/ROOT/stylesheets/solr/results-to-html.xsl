...
<xsl:stylesheet exclude-result-prefixes="#all" version="2.0"
  ...
  <xsl:template match="lst[@name='facet_fields']/lst"
    ...
    <xsl:element name="section">
      <xsl:if test="contains(@name,'iconography_scenes') or contains(@name,'administrative')">
        <xsl:attribute name="style">margin-top: 20%; border-top: 1px solid #ccc;</xsl:attribute>
    </xsl:if>
