...
<xsl:stylesheet version="2.0" xmlns:kiln="http://www.kcl.ac.uk/artshums/depts/ddh/kiln/ns/1.0"
  ...
  <xsl:template match="doc" mode="bibl-list">
    ...
    <xsl:choose>
      ...
      <xsl:when test="str[@name = 'concordance_bibliography_listed']">
        <tr>
          <xsl:if test="str[@name='concordance_bibliography_type']!='abbreviations'">
            <td>
  ...
  <xsl:template match="tei:bibl[@xml:id]" mode="full-citation">
    <xsl:apply-templates
      select="node() except (tei:title[@type = 'short'] | tei:bibl[@type = 'abbrev'])"/>
    ...
    <strong>
      <xsl:choose>
        ...
        <xsl:when test="tei:title[@type = 'short']">
          <xsl:apply-templates select="tei:title[@type = 'short'][1]"/>
        </xsl:when>
        <xsl:otherwise>
          <xsl:choose>
            <xsl:when test="ancestor::tei:div[@xml:id = 'authored_editions']">
              <xsl:for-each
                select="descendant::tei:name[@type = 'surname'][not(parent::*/preceding-sibling::tei:title[not(@type = 'short')])]">
                <xsl:value-of select="."/>
            ...
            <xsl:when test="descendant::tei:surname and descendant::tei:date">
              <xsl:for-each
                select="descendant::tei:surname[not(parent::*/preceding-sibling::tei:title[not(@type = 'short')])]">
                <xsl:value-of select="."/>
                <xsl:if test="position() != last()"> – </xsl:if>
              </xsl:for-each>
              <xsl:if test="descendant::tei:date/text()">
  ...
  <xsl:template match="tei:bibl[@type = 'abbrev'] | tei:title[@type = 'short']">
    <xsl:value-of select="."/>
  </xsl:template>

  <xsl:template match="tei:title[not(@type = 'short')]">
    <i>
      <xsl:value-of select="."/>
    </i>
  </xsl:template>
